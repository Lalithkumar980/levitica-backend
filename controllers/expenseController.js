...
async function create(req, res) {
  try {
    ...
    const payload = {
      ...
      recurring: ["No", "Yes Monthly", "One-off"].includes(body.recurring)
        ? body.recurring
        : "One-off",
    ...
    try {
      ...
      console.error("Failed to log expense finance activity:", err);
    }
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to create expense" });
  }
}
...
async function update(req, res) {
  try {
    ...
    if (body.status !== undefined)
      doc.status = body.status === "Pending" ? "Pending" : "Paid";
    ...
    if (body.receiptNo !== undefined)
      doc.receiptNo = String(body.receiptNo).trim();
    if (
      body.recurring !== undefined &&
      ["No", "Yes Monthly", "One-off"].includes(body.recurring)
    )
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to update expense" });
  }
}
...
async function remove(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to delete expense" });
  }
}
